...
- **La garantie dépend du client — et le repli est nommé.** L'élicitation n'existe que si le client MCP la supporte. Sur un client qui ne la supporte pas — et si la garde de présence physique (Touch ID, active par défaut) est désarmée —, `grant_channel` retombe sur le dialogue de permissions du client : celui-là même du problème initial, cadré par le LLM. La question rédigée par le serveur disparaît alors, et il ne reste que le plafond. C'est un repli documenté, pas un oubli : le statut du serveur dit désormais, **canal par canal**, si un accès a été confirmé par un humain ou auto-accordé.
